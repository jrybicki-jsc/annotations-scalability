...
def create_constrains(session):
    session.run(
        "CREATE constraint on (target:Target) assert target.jsonld_id is unique;")
    session.run(
        "CREATE constraint on (body:Body) assert body.jsonld_id is unique;")
